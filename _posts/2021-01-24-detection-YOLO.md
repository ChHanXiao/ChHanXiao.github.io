...
**主要贡献**：YOLOv1进行改进，大幅提升了YOLOv1的定位准确度和召回率，同时速度也是极快。
...
- location prediction：基于网格左上角预测，其会限制在0~1范围内，可以使得前期更加稳定。
...
yolov3用CE计算所有正例负例conf loss，用CE计算正例的cls loss，用CE计算xy loss，用MSE计算wh loss

yolov4用CIou计算bbox loss

yolov5的bbox分支采用的是Giou loss
